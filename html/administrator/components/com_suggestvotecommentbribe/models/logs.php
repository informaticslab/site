...
class SuggestionsModellogs extends JModel
{
	var $_data;
	...
	function _buildQuery()
	{
		...
		if ($this->search)
		{
			$where[] = 'LOWER(description) LIKE \'%'. $this->search. '%\'';
		}
	...
	function __construct()
	{
		...
		$this->filter_order_Dir = $mainframe->getUserStateFromRequest( $option.'log.filter_order_Dir', 'filter_order_Dir', '', 'word' );
		$this->filter_order  = $mainframe->getUserStateFromRequest( $option.'log.filter_order', 'filter_order',  'ordering', 'cmd' );
		$this->filter_order  = $mainframe->getUserStateFromRequest( $option.'log.filter_order', 'filter_order',  'ordering', 'cmd' );
		...
		$limitstart = $mainframe->getUserStateFromRequest( $option.'log.limitstart', 'limitstart', 0, 'int' );
