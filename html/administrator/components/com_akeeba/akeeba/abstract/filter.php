 ...
 * @version $Id: filter.php 299 2010-11-16 12:26:46Z nikosdion $
 */
...
abstract class AEAbstractFilter extends AEAbstractObject
{
	...
	public final function isFiltered($test, $root, $object, $subtype)
	{
		...
		if( $this->subtype == 'inclusion' ) return false;

		// The object and subtype must match
		if( ($this->object != $object) || ($this->subtype != $subtype) ) return false;
				...
				else
				{
					...
					foreach($this->filter_data[$root] as $regex)
					{
						if(substr($regex,0,1) == '!')
						{
							...
							if( !preg_match( substr($regex,1), $test) ) return true;
						}
						else
						{
							// Normal PCRE
							if( preg_match($regex, $test) ) return true;
						}
					}
