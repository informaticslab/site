 ...
 * @version $Id: fsfilter.php 284 2010-11-02 12:36:54Z nikosdion $
 * @since 3.0
 */
 ...
 *
 */
class AkeebaModelFsfilter extends JModel
{
	/**
	...
	private function &get_listing($root, $node)
	{
		...
		if(($vparts[0] == 5) && ($vparts[1] == 2) && ($vparts[2] <= 5) )
		{
		...
		if(!empty($files))
		{
			foreach($files as $file)
			{
				...
				$status['size'] = @filesize($directory.DS.$file);
	 ...
	 * @return array
	 */
	public function set($root, $crumbs, $item, $filter)
	{
		if(empty($item)) return array(
			'success' => false,
			'newstate'=> false
