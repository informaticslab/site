...
class modBreadCrumbsPlusHelper
{
	function getList(&$params)
	{
		...
		//always add "Home" to start of b.crumb
		$item = new stdClass();
		...
		if (!$params->get('showHome')) {
			...
			else { // We are on the home page so o/p a non-brkg. space to stop module posn collapsing
				$items[0]->name = '&nbsp;';
				$items[0]->link = '';
			}
		}
		
		if ((!$params->get('showComponent'))&&($count > 1)) {
			//hide component (=last) item from pathway 
			$items[$count-1]->name = '';
			$items[$count-1]->link = '';
		}
